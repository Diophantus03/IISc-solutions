...
import Mathlib.Data.Real.Basic -- random import

/-

# Basic Number Theory

I will work through a bunch of examples,
...
example (a b : ℕ) (h : a + b = 37) : (a : ℤ) + b = 37 := by
  /-
  a b : ℕ
  h : a + b = 37
  ⊢ ↑a + ↑b = 37
...
example (a b : ℕ) : ((a + b : ℕ) : ℤ)^2=(a : ℤ)^2+2*↑a*↑b+(b : ℤ)^2 := by
  -- ⊢ ↑(a + b) ^ 2 = ↑a ^ 2 + 2 * ↑a * ↑b + ↑b ^ 2
...
example (n : ℕ) (hn : 0 < n) : (n + 1) ∣ (n^2 + 1) ↔ n = 1 := by
  -- sorry
  refine ⟨?_, ?_⟩
  { rintro ⟨c, hc⟩ -- definitional abuse : `a ∣ b` is *defined* to mean `∃ c, b = a * c`
    zify at hc hn ⊢ -- I want to work with integers
    have h2 : ((n : ℤ) + 1) ∣ 2
    { use c - (n - 1)
      linear_combination hc } -- found with `polyrith` tactic
    have h3 : (n : ℤ) + 1 ≤ 2 := Int.le_of_dvd zero_lt_two h2 -- found with `library_search`
    linarith }
  { rintro rfl
    norm_num }
